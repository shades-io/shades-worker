...
const create = ({
    ...
    const handleEvent = event => {
        ...
        const storeProjectionIfNeeded = res => {
            if (!res.applied) {
                return res;
            }
        ...
        const publishDeltaIfNeeded = res => {
            if (!res.applied) {
                return {};
            }
            return broker.updates.publish({
                namespace,
        ...
        const process = processor =>
            loadProjection(processor.projectionName)
                ...
                .then(storeProjectionIfNeeded)
                .then(publishDeltaIfNeeded);

        return storeEvent()
            .then(() => Promise.all(processors.map(process)));
    };
